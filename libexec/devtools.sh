...
VERSION="0.1.0"
DATE="2014-12-21"
VCSVERSION="master@f1f0fcb77b7567b07175f7bf3e47da9745012ec5"
...
##@ ACTION_VARS = ( ACTION_NAME ACTION_VERSION ACTION_SYNOPSIS ACTION_DESCRIPTION ACTION_OPTIONS ACTION_CFGVARS ACTION_ADDITIONAL_INFO ACTION_ALLOWED_OPTIONS ACTION_ALLOWED_LONG_OPTIONS ) (read-only)
declare -rxa ACTION_VARS=(ACTION_NAME ACTION_VERSION ACTION_SYNOPSIS ACTION_DESCRIPTION ACTION_OPTIONS ACTION_CFGVARS ACTION_ADDITIONAL_INFO ACTION_ALLOWED_OPTIONS ACTION_ALLOWED_LONG_OPTIONS)
...
load_actions_infos () {
    ...
    for action in ${_BASEDIR}/*.sh; do
        ...
        if [ -n "$ACTION_ALLOWED_OPTIONS" ]; then
            ACTIONS_ALLOWED_OPTIONS["$pos"]="$ACTION_ALLOWED_OPTIONS"
        fi
        if [ -n "$ACTION_ALLOWED_LONG_OPTIONS" ]; then
            ACTIONS_ALLOWED_LONG_OPTIONS["$pos"]="$ACTION_ALLOWED_LONG_OPTIONS"
        fi
    done
    for section in "${ACTION_VARS[@]}"; do
        eval "export ${section}"
...
##@ ACTION_LIST ACTIONS_SYNOPSIS ACTIONS_OPTIONS ACTIONS_DESCRIPTION ACTIONS_ALLOWED_OPTIONS ACTIONS_ALLOWED_LONG_OPTIONS
# all actions vars
declare -xa ACTIONS_LIST=()
declare -xa ACTIONS_SYNOPSIS=()
declare -xa ACTIONS_OPTIONS=()
declare -xa ACTIONS_DESCRIPTION=()
declare -xa ACTIONS_ALLOWED_OPTIONS=()
declare -xa ACTIONS_ALLOWED_LONG_OPTIONS=()
...
action_exists () {
    ...
    action_file "$ACTION"
    if ( [ -z "$ACTION_FILE" ] || [ ! -f "$ACTION_FILE" ] ) && [ "$THROWERROR" = 'true' ]; then
...
load_action () {
    ...
    if [ ! -z "$ACTION_INDEX" ]; then
        ...
        if [ ! -z "${ACTIONS_ALLOWED_OPTIONS[$ACTION_INDEX]}" ]; then
            OPTIONS_ALLOWED="${ACTIONS_ALLOWED_OPTIONS[$ACTION_INDEX]}${OPTIONS_ALLOWED}";
        fi
        if [ ! -z "${ACTIONS_ALLOWED_LONG_OPTIONS[$ACTION_INDEX]}" ]; then
            LONG_OPTIONS_ALLOWED="${ACTIONS_ALLOWED_LONG_OPTIONS[$ACTION_INDEX]},${LONG_OPTIONS_ALLOWED}";
        fi
    fi
...
#### show_help ()
show_help () {
    if [ "$HELP_LESS" = 'true' ] || [ "$HELP_MORE" = 'true' ]; then
        local tmp_file=$(get_tempfile_path devtoolsusage)
    fi
    if [ -z "$ACTION" ]
    then
        if [ "$HELP_LESS" = 'true' ] || [ "$HELP_MORE" = 'true' ]
...
#    if [ "$_done" != 'true' ]; then cat "${tmp_file}"; fi
    exit 0
}
...
selfCheckAction () {
    ...
    then
        echo "A new version is available ...  You should run '$0 self-update [opts]' to get last version."
        return 1
    else
...
if [ "$DEBUG" = 'true' ]; then library_debug "$*" && echo; fi
OPTIND=1
